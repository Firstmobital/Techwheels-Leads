  ...
  return (
    <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
          ...
          <button
            ...
            className={cn(
              ...
              selectedOutcome === o.value
                ...
                : 'text-gray-500 bg-gray-50 border-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600'
            )}
          >
            {o.label}
          </button>
      ...
      <textarea
        ...
        className="w-full rounded-lg border border-gray-200 dark:border-gray-600 px-2.5 py-1.5 text-xs text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-200 resize-none"
      />
      <UIButton
        size="sm"
        onClick={handleSave}
        ...
        className={cn(
          ...
          saved ? 'bg-green-500 text-white' : 'bg-gray-900 dark:bg-gray-100 dark:text-gray-900 text-white'
        )}
      >
...
export default function LeadCard({ lead, tab, accentColor, message, isSent, onMarkSent, templates, sentMessages = [] }) {
  ...
  return (
    <div className={cn(
      "rounded-2xl shadow-sm border transition-all bg-white dark:bg-gray-800 dark:border-gray-700 overflow-hidden",
      allDone ? "border-gray-200 dark:border-gray-700 opacity-70" : nextDue?.overdue ? "border-red-200 dark:border-red-900" : "border-gray-100",
      urgencyBorder
    )}>
      <div className="p-4">
        {/* Header row */}
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              {allDone && (
                <span className="flex items-center gap-1 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-2 py-0.5 rounded-full flex-shrink-0">
                  <CheckCircle2 className="w-2.5 h-2.5" />
                  Done
                </span>
              )}
              {nextDue?.overdue && (
                <span className="flex items-center gap-1 text-[10px] font-semibold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 px-2 py-0.5 rounded-full flex-shrink-0">
                  <Clock className="w-2.5 h-2.5" />
                  {overdueDays && overdueDays > 0 ? `${overdueDays}d overdue` : 'Overdue'}
                </span>
              )}
              {!allDone && !nextDue?.overdue && nextDue?.daysUntil === 0 && (
                <span className="text-[10px] font-semibold text-orange-600 bg-orange-50 dark:bg-orange-900/30 px-2 py-0.5 rounded-full flex-shrink-0">
                  Due today
                </span>
              )}
                    ...
                    <React.Fragment key={i}>
                      <div className={cn(
                        ...
                        isDone
                          ...
                          ? 'bg-red-50 border-red-400 text-red-600 dark:bg-red-900/30'
                          : isNext && nextDue?.daysUntil === 0
                          ? 'bg-orange-50 border-orange-400 text-orange-600 dark:bg-orange-900/30'
                          : isNext
                          ? 'bg-blue-50 border-blue-400 text-blue-600 dark:bg-blue-900/20'
                          : 'bg-gray-50 border-gray-200 text-gray-400 dark:bg-gray-700 dark:border-gray-600'
                      )}>
                        {isDone ? '✓' : `M${i + 1}`}
                      </div>
                      {i < totalSteps - 1 && (
                        <div className={cn('h-px flex-1', isDone ? 'bg-emerald-300' : 'bg-gray-200 dark:bg-gray-600')} />
                      )}
                    </React.Fragment>
                  );
                })}
            ...
            <div className="flex items-center gap-1.5 flex-wrap mt-1.5">
              ...
              {(isGreenForms ? resolvedGreenFormOwnerId : normalizedLead.assigned_to) && (
                <span className="text-[10px] font-medium text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 px-2 py-0.5 rounded-full">
          ...
          {((!isTemplateDrivenTab && !allDone) || isTemplateDrivenTab) && (
            <div className="flex items-start gap-2 flex-shrink-0">
              <UIButton
                onClick={handleCall}
                variant="outline"
                className="rounded-xl h-12 w-12 p-0 shadow-sm dark:border-gray-600"
                aria-label="Call"
                title="Call"
                disabled={!callLink}
              >
